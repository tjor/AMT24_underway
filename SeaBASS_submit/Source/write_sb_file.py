...
def hdr(amt, fn_cal, fn_docs, model='ACS'):
   ...
   # if model == 'ACS':
    ...
    #elif model == 'ACS2':
     #   i_good = amt['acs2_ap'].values[:,10] != -9999
...
def hdr_hplc(amt, fn_docs):
    ...
    "/data_file_name=": "",
    "/documents=": fn_docs,     
    "/calibration_files=":'AMT24_Pigments_methods.docx', 
    "/data_type=": "pigment",
    "/data_status=": "preliminary",
    "/start_date=": "yyyymmdd",
    "/end_date=": "yyyymmdd",
    ...
    _fields ='sample,station,depth,lat,lon,year,month,day,time,volfilt,allo,alpha-beta-car,asta,but-fuco,Chl_a,Chl_c1,chl_c2,chl_c3,chlide_a,diadino,diato,dp,dv_chl_a,fuco,hex-fuco,lut,neo,perid,ppc,pras,psc,psp,tacc,tcar,tchl,tot_chl_a,tot_chl_b,tot_chl_c,tpg,viola,zea,beta-beta-Car,beta-epi-Car' # caution: copied from hdr function
    _units = 'none,none,m,degrees,degrees,yyyy,mo,dd,HH:MM:SS,l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l,ug/l'
...
def data_table_hplc(amt):
   ...
   # hplc_dhi_id = amt['hplc_dhi_no.'].to_pandas()
    ...
    station = amt['hplc_ctd_number'].to_pandas() # for AMT 24, `staion' is `ctd_number'
   ...
   # bottle = amt['hplc_bottle'].to_pandas()
    ...
    volfilt = amt['hplc_volume_filtered_(l)'].to_pandas() # units = liters
      

    # pigments
    allo = amt['hplc_Allo'].to_pandas()
    alpha_beta_car  = amt['hplc_Alpha-beta-Car'].to_pandas()
    Asta =  amt['hplc_Asta'].to_pandas()
    but_fuco = amt['hplc_But-fuco'].to_pandas()
    Chl_a = amt['hplc_Chl_a'].to_pandas() 
    Chl_c1 = amt['hplc_Chl_c1'].to_pandas()
    #Chl_c1c2 = amt['hplc_Chl_c1c2'].to_pandas() - missing in amt24
    Chl_c2  =  amt['hplc_Chl_c2'].to_pandas()
    ...
    #mv_chl_a = amt['hplc_MV_Chl_a'].to_pandas() - not present in AMT26
   # mg_DVP = amt['hplc_Mg_DVP'].to_pandas() missing in amt24mg_
    # mv_chl_b - not present in AMT28
    neo = amt['hplc_Neo'].to_pandas()
    perid = amt['hplc_Perid'].to_pandas()
    #phide_a = amt['hplc_Phide_a'].to_pandas() - not present in AMT26
   # phytin_a = amt['hplc_Phytin_a'].to_pandas() -not present in AMT26
    ...
    amt2csv = pd.concat([sample, station, depth, lat, lon, year, month,day,time ,volfilt, allo, alpha_beta_car, Asta, but_fuco, Chl_a, Chl_c1, Chl_c2, Chl_c3, Chlide_a, diadino, diato, DP, dv_chl_a, fuco, hex_fuco, lut, neo, perid, ppc, pras, psc, psp, tacc, tcar, tchl, tot_chl_a, tot_chl_b, tot_chl_c, tpg, viola, zea, beta_beta_Car, beta_epi_Car],  axis=1)
...
if __name__ == '__main__':
        ...
        # nc file #
        finalnc = '/users/rsg/tjor/scratch_network/AMT_underway/AMT24/Processed/Uway/Step3/amt24_final_with_debiased_chl.nc' # hardcoded
        amt_no = finalnc.split("/")[-1].split("_")[0]
        
        # calibrati  on file
        fn_cal_acs = 'acs122.dev' 
        fn_cal_acs_2 = 'acs167.dev' 
        #fn_cal_ac9 = 'ac90277.dev' 
        
        # document files
        fn_docs_acs = 'checklist_acs_particulate_inline_AMT24.rtf,checklist_acs_ag_cg_AMT24.rtf,AMT24_ACS_inline_ProcessingReport.docx' # hardcoded
        fn_docs_acs_2 = 'checklist_acs_particulate_inline_AMT24.rtf,checklist_acs_ag_cg_AMT24.rtf,AMT24_ACS_inline_ProcessingReport.docx' # hardcoded - note filenames are same as first acs, but text can differ
        ...
        fn_docs_hplc = 'checklist_hplc_PML_AMT24.rtf,AMT24_Pigments_methods.docx,AMT_24_PigmentsDen2.xlsx,AMT_24_Pigments_metadata.xlsx'
        ...
        amt2csv_acs2 = data_table_acs2(amt)
        ...
        # write file
        fnout_acs2 = '../sb_processed/' + header_acs2['/data_file_name=']
        export_2_seabass(header_acs2, amt2csv_acs2, fnout_acs2)
